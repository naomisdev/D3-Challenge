// I worked with Erin Wills (TA) and the following people:  Corrine, Nicole, Rui, David F, Rob W, Beth , Paule, Kena
...
function xScale(dataset, chosenXAxis) {
  ...
  function updateToolTip(chosenXAxis, circlesGroup) {
    ...
    var toolTip = d3.tip()
      ...
      .offset([80, -60])
      .html(function(d) {
        return (`${d.state}<br>${label} ${d[chosenXAxis]} %<br>Poverty>: ${d.poverty} %`); //circle hover info
      });
...
d3.csv("assets/data/data.csv").then(function(dataset, err) {
  ...
  dataset.forEach(function(data) {
    ...
    data.abbr = data.abbr;
    //data.num_albums = +data.num_albums; //xtra y Axis
  });
  ...
  var yLinearScale = d3.scaleLinear()
    .domain([d3.min(dataset, d => d.poverty)*0.90, d3.max(dataset, d =>d.poverty) *1.1 ])
    .range([height, 0]);
  ...
  var circlesGroup = updateToolTip(chosenXAxis, textcirclesGroup);
